...
| `TST1112` | warning  | `color-scheme` is declared but isn't the first mode dimension                                                                                     | List it first. Only the first dimension carries light/dark polarity, so a later `color-scheme` silently never reaches any exporter                                                    |
