...
| `TST1204` | info     | A role's `.solid` (authored in the default mode) has no override for a non-default `color-scheme` value — the whole derived grid carries over unchanged | Author `<role>.solid` for that scheme value if it should differ. Default behavior, not a mistake — `derivation.autoDark` would be the alternative, but isn't implemented yet          |
...
You didn't author a dark value, so it falls back to the light one — by design, and surfaced as `TST1204` (`info`) so it's visible in the report rather than only here. Transtyle does not invent brand decisions: author `primary.solid` a dark-mode value (one line). `derivation.autoDark` is specced to synthesize one instead, but isn't implemented yet ([roadmap](/docs/roadmap/#specced-not-yet-implemented)) — setting it today changes nothing.
