...
**See it rendered:** [demo/](demo/) shows the same fake app themed by the built `dist/` outputs in daisyUI and shadcn (terminal-dark by default), with a cross-link to Acme's demo for contrast. Run `npm run example:demo` (→ http://localhost:8123/cathode/demo/) — it refreshes the derived demo CSS from `dist/` and serves `examples/`; `demo/index.html` also opens straight from disk.
