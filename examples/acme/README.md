...
Note: `expected/` holds the *hand-written* artifacts from the Phase 0 paper exercises, kept for comparison — `expected/shadcn/` from [round 1](../../docs/exercises/phase0-shadcn.md), `expected/bootstrap/` from [round 2](../../docs/exercises/phase0-bootstrap.md), and `expected/storybook/` from [round 3](../../docs/exercises/phase0-storybook.md) (no Bootstrap or Storybook exporter exists yet; those files are the specification their implementations will be checked against). `dist/` is real compiler output. Small value differences between them are expected (the hand-runs used approximated color math) and documented in the exercise reports.
