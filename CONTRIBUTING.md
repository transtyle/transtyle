...
**`npm run check:demos`** ([scripts/check-demos.mjs](scripts/check-demos.mjs)) is the same rule for the demo harness, which is now a published surface: every example × target demo must exist, be described in [scripts/lib/demos.mjs](scripts/lib/demos.mjs), be listed with its port in its example's `demo/README.md`, be linked from its exporter page ("See it live"), be built by the CI matrix, and be deployed by [pages.yml](.github/workflows/pages.yml). It found two drifts on its first run: three demo READMEs a target behind, and two Storybook demos bound to another example's port.
