...
| 5 | Component-archetype groups (`formField`, `list`, `navigation`, `overlay`, `content`) | **New, but small and derivation-only.** Five semantic groups that mostly alias *existing* catalog values — see §4. Genuinely new vocabulary, but plumbing, not new primitives. |
...
1. **Ship the five new semantic groups (`field`, `list`, `navigation`, `overlay`, plus the two `text` hover cells) as an ordinary semantic-catalog addition** — no gate, same category of change as T7/T8.
2. **Build the generic severity-grid mapper once** (§5.2), proven against Button first since it's the component already verified against source in this document and it alone exercises the full grid.
3. **Extend the mapper across the ~25–35 severity-colored components via shape descriptors, not full prototypes** — this is the step that actually tests "does this scale," and produces the real evidence ADR-0003 asks for ("hand-written component-theming prototypes... generalize from evidence") faster than a single-component prototype would, since it stress-tests the *generic* mapper against real shape variance (some components have 3 variants, some have 5; some lack a `text` variant; Tag has no hover state at all).
4. **Handle the structural residue (DataTable, Galleria, Tree, …) opportunistically**, deriving their dimensional tokens from the five archetype groups and leaving genuinely bespoke parts `dropped`/`unsupported` with an honest coverage note, the same pattern every exporter already uses for target-specific concepts the IR doesn't model.
5. **Write the actual RFC/plan doc** (`docs/plan/component-tier.md`, mirroring `catalog-revision.md`'s task-by-task rigor) only after steps 1–4 produce real findings — some of what's guessed above (the exact radius-scale translation, which components truly lack a shape variant) will turn out wrong or incomplete once real component tokens are being resolved, and that plan should be written from what actually happened.

## Open questions for you

- Confirm the strategy in §5: override a base preset (Aura by default, configurable) + one generic severity mapper + per-component shape descriptors, rather than hand-authoring per-component value tables. This is the piece that makes "all ~90 components" tractable — flag now if you want a different approach.
...
- Do you want me to proceed to implementation (starting with the five new semantic groups + the generic mapper + Button), or write the formal plan doc first and pause for your sign-off before touching code?
