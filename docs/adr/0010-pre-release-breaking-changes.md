...
**What changed.** The project is publishing its first release to npm as an experimental alpha (ROADMAP AL6). Read literally, decision 4 would arm the freeze the moment that lands: from `0.1.0-alpha.0` onward the catalog would be additive-only, and any vocabulary mistake found during the alpha could only be fixed by carrying a permanent alias. **Maintainer decision: it does not.** "First npm publication" in decision 4 means **the first release whose version carries no prerelease identifier**. Prerelease versions (`0.1.0-alpha.N`) leave the pre-release regime of this ADR fully in force.

The trigger is defined on the _version_, deliberately, rather than on the `latest` dist-tag. npmjs.org auto-assigns `latest` on a package's very first publish regardless of the `--tag` passed, and `latest` cannot afterwards be removed — so for a package whose first-ever release is an alpha, `latest` pointing at that alpha is an artifact of the registry, not an act of promotion, and a tag-based trigger would fire on it. A version-based trigger cannot be tripped by registry behavior: it fires only when a human decides to cut a release without a prerelease identifier.

**Why this is the same decision, not a new one.** Decision 4 exists because publication creates users with a legitimate compatibility interest — that is the whole reason the discipline returns. An alpha creates that interest far more weakly, and the original context line still applies almost unchanged: backward compatibility has near-zero value and real cost (a permanent double vocabulary), and the cost of getting the vocabulary wrong is highest precisely when the first outside users are about to report it.

**What this does _not_ rest on.** It is tempting to argue that alpha users have all opted in explicitly. They have not, and the policy should not pretend otherwise: while no stable release exists, the `latest` dist-tag deliberately tracks the newest alpha (see [RELEASING.md](../../RELEASING.md)), so a bare `npm install @transtyle/cli` installs an alpha. That is a considered choice — the alternative strands `latest` on whichever prerelease npm auto-tagged first, which serves nobody — but it means obscurity is not doing any of the work here. What is doing the work: every version carries an `-alpha.N` identifier, which appears in the installed `package.json` and in every lockfile; the README and the site say what that means in the first screen; and `@alpha` exists as a channel to pin to. The protection is disclosure, not difficulty of access.
...
**The obligation this exemption creates.** The exemption is only honest while the experimental status is impossible to miss. It is contingent on all four of: every alpha version carrying an `-alpha.N` prerelease identifier; the `alpha` dist-tag being kept current, so there is a channel to pin to; the README carrying the alpha warning; and the website carrying its site-wide alpha banner. The first of those is what a lockfile records and what a semver range is checked against, so it is the one that does the real work. If a release is ever cut without a prerelease identifier, the freeze arms at that moment with no further decision required.
