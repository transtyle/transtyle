...
<!-- measured: cathode.primeng.driven = 79 -->
<!-- measured: cathode.primeng.inherited = 1552 -->
<!-- measured: cathode.primeng.base = 1128 -->
<!-- measured: acme.primeng.driven = 80 -->
<!-- measured: acme.primeng.base = 1127 -->

Current split: **79 driven · 1552 inherited · 1128 on Aura's default** on Cathode, GOV.UK and Carbon — and **80 driven · 1552 inherited · 1127** on Acme. The one-slot difference is the interesting part. This split used to be identical everywhere, because it depended only on the exporter; it now depends on the design system too, in exactly the places the component tier reaches. Acme authors `component.tooltip.max-width`, PrimeNG binds it to `components.tooltip.root.maxWidth`, and that slot moves from Aura's default to driven. A design system that authors no component tier keeps the 79. The bar earned its keep immediately: it exposed that `semantic.typography.*` was left on Aura's defaults, which cascaded to 60 component slots referencing `{typography.font.size}`/`{typography.font.weight}`. Driving four values from the IR's type scale converted 64 slots and made PrimeNG components render in the design system's own typeface and base size (Carbon's PrimeNG demo is now IBM Plex Sans at the Carbon base size, not Aura's `inherit`/0.875rem). Matching is deliberately **exact**: a prefix rule ("we drive some `formField.*`, so `{form.field.font.size}` counts") was implemented, measured to over-claim 221 slots, and removed — inflating coverage is the one outcome this bar exists to prevent.
