...
| `demo/storybook` | Storybook (`html-vite`) + Tailwind (for sibling daisyUI CSS) | The simplest possible Storybook wearing the theme: a couple of component stories + a live token-swatch story | `.storybook/manager.ts` imports `manager.transtyle.ts` (themed chrome, DS-native mode); `.storybook/preview.ts` re-exports `preview.transtyle.ts` (sibling stylesheets, Scheme toolbar, mode decorator, DS backgrounds) |
...
- **npm-launchable.** Every project is a workspace: `npm run dev -w <example>-demo-<target>`. Ports: Acme 4101–4104 + 6101 (Storybook); Cathode 4201–4204 + 6201.
