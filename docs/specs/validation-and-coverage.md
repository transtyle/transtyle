...
- **Tier violations** — a semantic token aliasing a component token, an exporter binding below the semantic tier. Only the coarser "top-level group isn't a tier name" check exists today (`TST1305`). Tracked as [issue #8](https://github.com/transtyle/transtyle/issues/8).
