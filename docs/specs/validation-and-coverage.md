...
- **Ground-truth tests per exporter:** generated output is loaded by the _actual target toolchain_ (compile the Sass with Bootstrap, apply the ECharts theme in headless render, boot the themed Storybook) — snapshot tests catch our regressions; ground-truth tests catch the framework moving underneath us.
