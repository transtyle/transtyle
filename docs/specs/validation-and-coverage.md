...
|---|---|---|
| `native` | Target has a first-class slot; value passes through losslessly | `semantic.color.primary.base → $primary` |
...
- **Property tests on derivation:** e.g. contrast-pick always returns the max-contrast candidate; scales are monotonic; OKLCH ramps stay in gamut after clamping.
