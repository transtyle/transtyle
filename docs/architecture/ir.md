...
- **Custom roles** may declare an *archetype* (`brand`, `status`, `neutral`) via `$extensions.transtyle.role: { "archetype": "..." }` on the role's group (alongside its `.solid` child, not on the leaf itself) and get the full grid derived like a built-in role — the same generic derivation `derive.js` uses for `secondary`/`accent`/etc., just requiring the role's own `.solid` authored instead of computing it from `primary` (**implemented**, plan task T7; see Cathode's `crt-amber`, `docs/specs/exporters/css-variables.md` and `docs/specs/exporters/daisyui.md` for how open-role-set exporters emit it).
