...
- **Dimensions:** explicit units required (`16px`, `1rem`), and carried through **as authored** — there is no normalization pass for them, so `0.5rem` reaches the exporter as `0.5rem`. An exporter whose target demands another unit converts locally at a fixed 16px base (ECharts does this for radii) and reports the row `approximated` with the base named. **Specced:** a config-level `rem` base to replace that constant; unit conversion is flagged `approximated` in coverage when it changes meaning.
