...
4. **`CHANGELOG.md`** (new) — Keep a Changelog format; an _Unreleased_ section that honestly inventories what exists on `main` today (core/ir/cli, three exporters, two examples, website). Notes that nothing is on npm yet and that IR/plugin-API are versioned separately per `docs/architecture/versioning.md`.
