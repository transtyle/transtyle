...
While assembling this, `check:color` failed intermittently — it used `Math.random()`, making it flaky *and* blind: my B7 claim that hex "round-trips losslessly, 0 delta over 4096 samples" was luck. An exhaustive 16,777,216-value sweep shows **1580 values (0.0094%) drift by exactly 1/255**, all near-black. The check is now deterministic (fixed 4-step grid), asserts the true bound plus exact round-trips for colours anyone authors, and the behaviour is documented on `formatHex`. The B7 worklog carries the correction. Build determinism is unaffected.
