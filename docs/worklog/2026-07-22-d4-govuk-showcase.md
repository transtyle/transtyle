...
**One correction during verification:** two bars (daisyui, bootstrap) summed to 101% — the CLI's own rounded percentages carry 1% rounding slack. Trimmed the dominant `derived` segment by 1% in each so the *visual* bar is exactly 100; the prose keeps the exact reported figures. All eight bars now sum to 100.
