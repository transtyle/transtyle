...
The hero terminal is the one exception: `.terminal` already owns a rounded frame one level up (14px, `overflow:hidden`, wrapping the title bar _and_ the code together). Adding a second radius on its `.code-wrap` would double up right where the two meet, so `.terminal .code-wrap` is explicitly neutralised (`border-radius: 0; overflow: visible`) and inherits its shape entirely from the ancestor's clip.
