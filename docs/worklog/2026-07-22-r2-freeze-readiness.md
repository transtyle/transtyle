...
1. **ADR-0010 already moved the real freeze to first publication** and reserves pre-release breaking changes. So R2 can't literally "declare frozen now" — it's a _readiness audit_ that pre-verifies the v0 content so the R4 re-arm locks a known-good surface.
