...
## What it adds
`diff` now reports the **accessibility cost of a change**. `check` answers "is contrast bad now?"; diff answers "did *this change* make it bad?" — the reviewer's actual question, and the one a green CI baseline can silently regress on.
