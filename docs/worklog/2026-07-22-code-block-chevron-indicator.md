...
**Lesson carried forward:** the previous two attempts were both signed off on `getComputedStyle` assertions (opacity, background-image, box-shadow strings) without an actual screenshot — which is exactly how a "technically correct, practically invisible" fix slipped through twice. Screenshot before claiming done, every time, for anything about *visibility* rather than *behaviour*.
