...
- Re-ran the actual P2 acceptance path: extracted `write-an-exporter.md`'s code fences verbatim _after_ reformatting and ran them through `@transtyle/plugin-kit`'s conformance suite again — all 9 checks still pass. Formatting is cosmetic; behavior is unaffected, confirmed rather than assumed.
